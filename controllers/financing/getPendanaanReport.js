...
const getPendanaanReport = async (req, res) => {
  try {
    ...
    const query = `
      ...
      WHERE f.category = 'Pendanaan Syariah UMKM' AND f.status IN ('APPROVED', 'ACTIVE', 'COMPLETED', 'BAD_DEBT')
    ...
    const dibayarQuery = `
      ...
      WHERE f.category = 'Pendanaan Syariah UMKM' AND b.status = 'PAID' AND b.category_code = 'TRANSACTION_INSTALLMENT'
    `;
    ...
    const badDebtQuery = `
      SELECT 
        ...
        (
          ...
          WHERE fa.category = 'Pendanaan Syariah UMKM' AND fa.status = 'BAD_DEBT' AND b.status = 'PAID' AND b.category_code = 'TRANSACTION_INSTALLMENT' AND YEAR(b.updated_at) < :currentYear
        ) AS paidAwal,
        (
          SELECT COALESCE(SUM(b.amount), 0)
          FROM bill_items b
      ...
      FROM financing_applications f
      WHERE f.category = 'Pendanaan Syariah UMKM' AND f.status = 'BAD_DEBT'
    `;
    ...
    const jurnal_saldo_awal = {
      ...
      hutang: { debet: 0, kredit: 0, total: 0 },
      aktiva_tetap: { debet: 0, kredit: 0, total: 0 }
    };
...
export default getPendanaanReport;
